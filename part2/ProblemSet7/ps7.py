...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos2 = (x,y)
        if not pos2 in self.cleaned:    
            self.cleaned.append(pos2)
        return self.cleaned
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        if 0<= pos.getX() and pos.getX() < self.width and 0 <=  pos.getY()  and pos.getY()< self.height:
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        new_pos = current_pos.getNewPosition(self.dir, self.speed)
        if new_pos.getX() <=0 or new_pos.getX() >= self.room.width or new_pos.getY() <=0 or new_pos.getY() >= self.room.height:
            self.dir = random.randrange(360)
        else:
...
avg = runSimulation(10, 1.0, 20, 20, 0.8, 30, StandardRobot)
# === Problem 4
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        new_dir = random.randrange(360)
        self.setRobotDirection(new_dir)
        new_pos = current_pos.getNewPosition(new_dir, self.speed)
...
#       (... your call here ...)
#
#showPlot1('s1','times','simulation per number of robots')
#showPlot2('s2','times','simulation per aspect ratios of width/height')
#
